...
面向：希望「下次能自己启动」以及「从产品经理视角理解项目在做什么」的开发者/使用者。

---

## 一、如何启动项目（具体步骤）

### 1. MySQL 要不要先去 `bin` 里启动？

**一般不需要。**

在 Windows 上，MySQL 通常已安装为**系统服务**，开机或安装完成后会自动在后台运行。你只要确认：

1. 按 `Win + R`，输入 `services.msc` 回车，打开「服务」；
2. 找到 **MySQL** 或 **MySQL80** 之类名称的服务，状态为**正在运行**即可。

若服务已运行，**不必**每次先 `cd` 到 `D:\mysql\mysql-8.0.45-winx64\bin` 再开 PowerShell。  
只有在你是**手动 `mysqld` 启动、且未装成服务**时，才需要每次在 `bin` 目录下启动 MySQL（不推荐，建议把 MySQL 配成服务）。
...
（可选）把 `D:\mysql\mysql-8.0.45-winx64\bin` 加入系统环境变量 **PATH** 后，可在任意目录直接输入 `mysql`。

### 2. 启动本应用（每次开发/使用）

1. 确认 **MySQL 服务已运行**，且库 `rag_v3` 仍存在（你已创建过则不用重复建）。
2. 打开 **PowerShell** 或 **终端**。
...
**一句话**：基于 **Flask** 的 Web 应用，把用户上传的文档做 **解析 → 切片 → 向量化**，存进 **Chroma**；用户提问时做 **检索（向量 / 关键词 / 混合）**，把检索到的片段作为 **上下文**，交给 **大语言模型（LLM）** 流式生成回答，并尽量带上 **引用来源**。
